...
- OpenSpec repository files exist and validate successfully.
...
2. `openspec --version` reports the expected CLI version.
3. `bash scripts/validate.sh` completes successfully inside the container.
4. Authentication is performed interactively or with a short-lived secret, never committed to the repository.
